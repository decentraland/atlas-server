...
const component_1 = require("../../src/modules/api/component");
...
let rentalsComponentMock;
let subgraphComponentMock;
let defaultParcel;
let defaultFstParcelEstate;
let defaultSndParcelEstate;
let defaultParcelNFT;
let defaultFstParcelEstateNFT;
let defaultSndParcelEstateNFT;
let defaultParcelTile;
let defaultFstParcelEstateTile;
let defaultSndParcelEstateTile;
let fstEstateNFT;
beforeEach(async () => {
    defaultParcel = {
        id: 'parcel-0x0-0',
    ...
    defaultParcelTile = {
        ...
        price: 1,
        expiresAt: Math.round(parseInt(defaultFstParcelEstate.activeOrder?.expiresAt, 10) / 1000),
    ...
    defaultSndParcelEstateTile = {
        ...
        price: 2,
        expiresAt: Math.round(parseInt(defaultSndParcelEstate.activeOrder?.expiresAt, 10) / 1000),
    ...
    apiComponent = await (0, component_1.createApiComponent)({
...
describe('when fetching data', () => {
    describe('and fetching the graph fails', () => {
        ...
        it('should throw an error', () => {
            return expect(apiComponent.fetchData()).rejects.toThrowError('An error ocurred');
        });
    });
    ...
    describe("and there's more than one batch of nfts", () => {
        let nftFragments;
        let fourthParcel;
        let fifthParcelEstate;
        let sixthParcelEstate;
        let fourthParcelRentalListing;
        let fifthParcelEstateRentalListing;
        beforeEach(() => {
            fourthParcel = {
                ...defaultParcel,
                id: 'parcel-0x0-3',
            ...
            rentalsComponentMock.getRentalsListingsOfNFTs = jest
                ...
                [fifthParcelEstateRentalListing.nftId]: fifthParcelEstateRentalListing,
            })
                .mockResolvedValueOnce({});
        });
                ...
                tiles: [
                    {
                        ...
                        owner: defaultParcel.owner?.id,
                        price: 1,
                        expiresAt: Math.round(parseInt(defaultFstParcelEstate.activeOrder?.expiresAt, 10) /
                            1000),
                        tokenId: defaultParcel.tokenId,
                    },
                    {
                    ...
                    },
                    {
                        ...
                        owner: defaultSndParcelEstate.owner?.id,
                        price: 2,
                        expiresAt: Math.round(parseInt(defaultSndParcelEstate.activeOrder?.expiresAt, 10) /
                            1000),
                        tokenId: defaultSndParcelEstate.tokenId,
                    },
                    {
                        ...
                        rentalListing: (0, rentals_1.convertRentalListingToTileRentalListing)(fourthParcelRentalListing),
                    },
                    {
                        id: '4,3',
                        ...
                        price: 2,
                        expiresAt: Math.round(parseInt(fifthParcelEstate.activeOrder?.expiresAt, 10) / 1000),
                        tokenId: fifthParcelEstate.tokenId,
                        rentalListing: (0, rentals_1.convertRentalListingToTileRentalListing)(fifthParcelEstateRentalListing),
                    },
                    {
                        id: '5,3',
                        ...
                        price: 2,
                        expiresAt: Math.round(parseInt(sixthParcelEstate.activeOrder?.expiresAt, 10) / 1000),
...
describe('when fetching update data', () => {
    let fstEstate;
    let defaultParcelRentalListing;
    let fstEstateRentalListing;
    beforeEach(() => {
        fstEstate = {
            updatedAt: date.toString(),
            estate: {
    ...
    describe('and fetching the graph fails', () => {
        ...
        it('should throw an error', () => {
            return expect(apiComponent.fetchUpdatedData(0, {})).rejects.toThrowError('An error ocurred');
        });
    });
    ...
    describe('and there are only listings that belong to a parcel to be be updated', () => {
        ...
        it('should return a batched list of tiles to update with their rental listings updated', () => {
            return expect(apiComponent.fetchUpdatedData(100000, {
                [defaultParcelTile.id]: defaultParcelTile,
            })).resolves.toEqual({
                tiles: [
                    {
                        ...defaultParcelTile,
                        rentalListing: (0, rentals_1.convertRentalListingToTileRentalListing)(defaultParcelRentalListing),
                    },
                ],
                parcels: [],
                estates: [],
        ...
        it('should return a batched list of tiles to update with their rental listings updated', () => {
            return expect(apiComponent.fetchUpdatedData(100000, {
                ...
                tiles: [
                    {
                        ...defaultFstParcelEstateTile,
                        rentalListing: (0, rentals_1.convertRentalListingToTileRentalListing)(fstEstateRentalListing),
                    },
                    {
                        ...defaultSndParcelEstateTile,
                        rentalListing: (0, rentals_1.convertRentalListingToTileRentalListing)(fstEstateRentalListing),
    ...
    describe('and there are only rental listings that belong to parcels that are not open anymore', () => {
        beforeEach(() => {
            ...
            rentalsComponentMock.getUpdatedRentalListings = jest
                .fn()
                .mockResolvedValueOnce([
                { ...defaultParcelRentalListing, status: schemas_1.RentalStatus.CANCELLED },
            ]);
        });
    ...
    describe('and there are only rental listings that belong to estates that are not open anymore', () => {
        beforeEach(() => {
            ...
            rentalsComponentMock.getUpdatedRentalListings = jest
                .fn()
                .mockResolvedValueOnce([
                { ...fstEstateRentalListing, status: schemas_1.RentalStatus.CANCELLED },
            ]);
        });
        it("should return a batched list of tiles to update with their rental listings removed when the rental listing wasn't open", () => {
            return expect(apiComponent.fetchUpdatedData(100000, {
    ...
    describe('and there are parcels and rental listings associated to those parcels to be updated', () => {
        let updatedDefaultParcel;
        let updatedDefaultParcelTile;
        let updatedDefaultParcelNFT;
        beforeEach(() => {
    ...
    describe('and there is a parcel that is added to an estate and a rental listing is created for that state', () => {
        let updatedFstEstate;
        let updatedDefaultParcelIntoFstEstate;
        let updatedDefaultParcelTileIntoFstEstate;
        beforeEach(() => {
